...
export default function Topbar() {
  ...
  return (
    <nav className="navbar bg-info mb-lg-10 p-2">
        ...
        {user ? (
          <div className="d-flex align-items-center">
            <Link className="link" to="/">
              <span className="mx-4 borderbtn px-3 py-1 text-muted">
                {user.username}
              </span>
            </Link>

            <span className="borderbtn px-3 py-1 text-muted" onClick={logout}>
              Logout
            </span>
          </div>
        ) : (
